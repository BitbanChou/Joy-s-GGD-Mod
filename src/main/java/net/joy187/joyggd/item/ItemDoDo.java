...
public class ItemDoDo extends Item {

    private static final Predicate<Entity> DODO_AREA = (entity) -> {
        return entity.isAlive() && !(entity instanceof PlayerEntity);
    };

    public ItemDoDo(Item.Properties p_41383_) {
        super(p_41383_);
    }
    ...
    public ActionResult<ItemStack> use(World level, PlayerEntity playerIn, Hand hand) {
        ...
        if(!playerIn.getCooldowns().isOnCooldown(itemstack.getItem()))
        {
            ...
            int radius=(20-brightness);
            for (LivingEntity livingentity : playerIn.level.getEntitiesOfClass(LivingEntity.class, playerIn.getBoundingBox().inflate(radius), DODO_AREA)) {
...
//                double d0 = 1.5D *(15-brightness)* (1.0D - livingentity.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
                ...
                livingentity.push(d0 / d2 * radius, 0.2D, d1 / d2 * radius);
    ...
    public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if(Screen.hasShiftDown()) {
            tooltip.add(new TranslationTextComponent("tooltip.dodo"));
