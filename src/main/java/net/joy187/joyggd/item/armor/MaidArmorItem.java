...
public class MaidArmorItem extends GeoArmorItem implements IAnimatable {
    private AnimationFactory factory = new AnimationFactory(this);

    public MaidArmorItem(CustomArmorMaterial materialIn, EquipmentSlotType slot, Properties builder) {
    ...
    private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
        ...
        if (livingEntity instanceof ArmorStandEntity) {
            ...
            boolean isWearingAll = armorList
                    .containsAll(Arrays.asList(ItemInit.MAID.get()));
    ...
    public void onArmorTick(ItemStack stack, World world, PlayerEntity player) {
        if(!world.isClientSide()) {
            if(!player.inventory.getArmor(2).isEmpty()) {
                ...
                if(chest.getMaterial() == CustomArmorMaterial.ARMOR_MATERIAL_LANTERN) {
                	if(player.getHealth()<5F)
                    {
                        player.addEffect(new EffectInstance(Effects.REGENERATION,100, 0));
                    }
...
//                    }
    ...
    public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new TranslationTextComponent("tooltip.maid"));
    }
}